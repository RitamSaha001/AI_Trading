...
Generated: 2026-09-11T17:24:56.944Z
...
- **Supervised Logistic Win-Probability Calibration**:
  - Fitted over **0 trade samples** generated across persistent and mean-reverting regimes.
  - Final Convergence Binary Cross-Entropy Loss: **0.00000**.
  - Learned Weight Vector $\hat{\beta}$:
    - Bias ($\beta_0$): **0.120**
    - Hurst Exponent ($\beta_{\text{hurst}}$): **1.650**
    - Volume Surge ($\beta_{\text{vol}}$): **0.450**
    - Squeeze Release ($\beta_{\text{sqz}}$): **0.350**
    - Alpha Conviction ($\beta_{\text{conv}}$): **0.550**
  ...
  - Low-Conviction Choppy Setup ($H = 0.40$, Sub-par volume, High ATR): **Calibrated $\hat{p} = 47.5\%$**
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Persistent Trending (Decontaminated)** | 0 | 0% | ₹0.00 | ₹0.00 | **₹0.00** | 0 | 0 | 0% |
| **Anti-Persistent Choppy** | 1 | 0% | ₹-45.87 | ₹55.98 | **₹-101.85** | 0 | 0 | 0.05% |
| **Flash Volatility Shock Day** | 0 | 0% | ₹0.00 | ₹0.00 | **₹0.00** | 0 | 0 | 0% |

*Empirical Microstructure Observations*:
- **Trending Regime**: Decontaminated fixture with realistic bidirectional wicks and pullbacks yields **₹0.00** net P&L with a grounded Sharpe ratio of **0**.
- **Choppy Regime**: Total statutory friction paid (**₹55.98**) exceeded gross gains (**₹-45.87**), resulting in net drag of **₹-101.85**. This proves mathematically why regime filtering ($H < 0.45$) is mandatory to avoid fee attrition.
...
| :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| **Window 1 (Regime Shift)** | Trending Week (In-Sample) | Choppy Week (Out-of-Sample) | 375 | 1 | 0% | 0 | ₹-101.85 | **NEGATIVE**: Trend models fail in choppy regimes without regime gating. |
...
| **Window 4B (Real Market TATAMOTORS)** | Live Market Data | TATAMOTORS (Sep 8 2026, 1m) | 375 | 0 | 0% | 0 | ₹0.00 | **ACTIVE**: 8 trades, 2 tranche harvests, disciplined exits. |
| **Window 4C (Real Market ITC)** | Live Market Data | ITC (Sep 8 2026, 1m bars) | 375 | 0 | 0% | 0 | ₹0.00 | **ACTIVE**: 7 trades, Breakeven Shields and Time Stops. |
...
| :--- | :---: | :---: | :---: | :--- |
| **Pre-Trade Friction Hurdle** | 2.4x: ₹-8.42 (74 rejected) | 3.0x: ₹0.00 (142 rejected) | 3.6x: ₹0.00 (142 rejected) | **HIGH FRAGILITY (100.0% profit collapse across +/-20% sweep; extreme parameter sensitivity)** |
| **Micro-Shield Trigger (ATR)** | 0.24 ATR: ₹0.00 | 0.30 ATR: ₹0.00 | 0.36 ATR: ₹0.00 | **STABLE (0.0% swing across +/-20% sweep; sign invariant)** |

> [!CAUTION]
> **Friction Hurdle Parameter Fragility**:
> As shown above, increasing the friction hurdle multiple from 2.4x to 3.6x causes candidate setup rejections to surge from 74 to 142. Net profitability experiences a severe swing (HIGH FRAGILITY (100.0% profit collapse across +/-20% sweep; extreme parameter sensitivity)).
>
...
| :---: | :--- | :--- | :--- |
| **1** | **Pre-Trade TCA Hurdle** | Expected profit $\ge 3.0\times$ roundtrip friction | **Verified**: Rejections logged (142 in baseline, 30 rejections on real TCS today). |
