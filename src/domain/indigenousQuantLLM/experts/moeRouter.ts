...
export class MoERouter {
  ...
  public static route(headline: string): { weights: MoERouterWeights; topExperts: string[] } {
    ...
    if (/PROFIT|REVENUE|NET|Q[1-4]|RESULTS?|EARNINGS?|BEATS?|MISSES?|EBITDA|MARGIN|ORDER|CONTRACT|DEAL|WINS?|BAGS?|SIGNS?/i.test(upper)) {
      lEarnings += 4.0;
    }
    if (/SEBI|ED\s+RAID|CBI|PROBE|NOTICE|PENALTY|\bBANS?\b|FRAUD|AUDIT|USFDA|WARNING|OBSERVATION/i.test(upper)) {
      lRegulatory += 4.5;
    }
